...
internal data class RedisConfig(
    ...
    companion object {
        fun fromEnvironment(env: Map<String, String> = System.getenv()): RedisConfig {
            ...
            val config = if (redisUrl.isNotBlank()) {
                ...
                val host = env["REDIS_HOST"]?.trim().orEmpty().ifBlank { "localhost" }
                val port = env["REDIS_PORT"]?.trim()?.toIntOrNull()?.coerceIn(1, 65535) ?: 6379
